...
const MESSAGE_FILE = "app/errors/errors.yaml"

func init() {
	err := loadMessages(MESSAGE_FILE)
	if err != nil {
		panic(err)
	}
}
...
func ExistError(err string) *APIError {
	return NewAPIError(http.StatusBadRequest, "EXIST_ERROR", Params{"error": err})
}
